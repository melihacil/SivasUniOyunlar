...
public class Dart4 : MonoBehaviour
{
    
    bool checker4 = false;
    
    public float speed = 100.0f;
    public GameObject dartArrow4;
    //dart 
    ...
    private void Awake()
    {
        m_Rigidbody = dartArrow4.GetComponent<Rigidbody>();
    }
    private void OnCollisionEnter(Collision collision)
    {

        if (collision.gameObject.tag == "2" && checker4 == false)
        {
            checker4 = true;
            skor.point2();
            Stop();
        ...
        else if (collision.gameObject.tag == "4" && checker4 == false)
        {
            checker4 = true;
            skor.point4();
            Stop();
        }
        else if (collision.gameObject.tag == "6" && checker4 == false)
        {
            checker4 = true;
            skor.point6();
            Stop();
        }
        else if (collision.gameObject.tag == "8" && checker4 == false)
        {
            checker4 = true;
            skor.point8();
            Stop();
        }
        else if (collision.gameObject.tag == "10" && checker4 == false)
        {
            checker4 = true;
            skor.point10();
            Stop();
        }
        else if (collision.gameObject.tag == "12" && checker4 == false)
        {
            checker4 = true;
            skor.point12();
            Stop();
