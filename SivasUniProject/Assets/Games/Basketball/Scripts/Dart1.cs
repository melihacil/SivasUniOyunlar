...
public class Dart1 : MonoBehaviour
{
    ...
    private void Awake()
    {
        m_Rigidbody = dartArrow1.GetComponent<Rigidbody>();
    }
    private void OnCollisionEnter(Collision collision)
    {

        if (collision.gameObject.tag == "2" && checker1 == false)
        {
            checker1 = true;
            skor.point2();
            Stop();
        ...
        else if (collision.gameObject.tag == "4" && checker1 == false)
        {
            checker1 = true;
            skor.point4();
            Stop();
        }
        else if (collision.gameObject.tag == "6" && checker1 == false)
        {
            checker1 = true;
            skor.point6();
            Stop();
        }
        else if (collision.gameObject.tag == "8" && checker1 == false)
        {
            checker1 = true;
            skor.point8();
            Stop();
        }
        else if (collision.gameObject.tag == "10" && checker1 == false)
        {
            checker1 = true;
            skor.point10();
            Stop();
        }
        else if (collision.gameObject.tag == "12" && checker1 == false)
        {
            checker1 = true;
            skor.point12();
            Stop();
